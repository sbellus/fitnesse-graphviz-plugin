...
public class GraphvizDotSymbol extends SymbolType implements Rule, Translation {
    ...
    public Maybe<Symbol> parse(Symbol current, Parser parser) {
        ...
        if (DotExecutable == null) {
            Symbol error = new Symbol(new Preformat(), "").add(
                    "The property \"graphviz.dotExecutable\" in plugins.properties MUST BE set and it is not set.");
        ...
        try {
            ...
            if (exitCode != 0) {
                ...
                Symbol error = new Symbol(new Preformat(), "").add("Process that converts dot to svg exits with code "
            ...
            Symbol error = new Symbol(new Preformat(), "")
                    .add("During dot picture generation following exception occures:\n" + exception.toString());
            return new Maybe<Symbol>(error);
        }
